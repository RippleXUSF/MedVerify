...
export default function DistributePage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch("/api/confirm", {
        ...
        body: JSON.stringify({ role: "distributor", tokenId: tokenId.trim() }),
      });
  ...
  return (
    <div style={{ minHeight: "100vh", backgroundColor: "#111312" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem" }}>
          ...
          >
            ...
            <p style={{ fontSize: 13, color: "#6E6C66", lineHeight: 1.5, marginBottom: "1rem" }}>
              Enter the batch token ID provided by the manufacturer. This records an on-chain
              confirmation that your facility received this batch.
            </p>
            ...
            {result && (
              ...
              >
                ...
                <div style={{ backgroundColor: "#1C1F1D", padding: 14 }}>
                  <InfoDividerRows
                    rows={[
                      { key: "Distributor address", value: result.actor },
          ...
          >
            ...
            <CustodyTimeline
              steps={[
                ...
                { role: "DISTRIBUTOR", label: "Confirm receipt", completed: false, current: true },
                { role: "PHARMACY", label: "Confirm & generate QR", completed: false },
              ]}
            />
            <div style={{ height: "0.5px", backgroundColor: "#272B29", margin: "1rem 0" }} />
            <p style={{ fontSize: 12, color: "#6E6C66", lineHeight: 1.6 }}>
              After confirming, pass the token ID to the pharmacy to complete the chain of custody.
            </p>
          </div>
        </div>
      </main>
      ...
      {showToast && (
        <Toast message="Distributor receipt confirmed on XRPL" onDone={() => setShowToast(false)} />
      )}
    </div>
  );
}
